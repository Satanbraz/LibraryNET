...
namespace LibraryNET.Controllers
{
    ...
    public class BooksController : Controller
    {
        ...
        public ActionResult BooksList(int bookGenreId = 0, string busqueda = "", int page = 1, int PageSize = 9)
        {
            int totalCount;
            IEnumerable<Books> books;

            if (bookGenreId > 0 || !string.IsNullOrEmpty(busqueda))
            {
                totalCount = _booksManager.ItemsCountFiltered(bookGenreId, busqueda);
                books = _booksManager.ListFiltered(bookGenreId, busqueda, page, PageSize);
            }
            else
            {
